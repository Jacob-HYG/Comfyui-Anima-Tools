...
def _remote_thumb_stable_cache_key(
...
# ----------------- 图片缓存系统 (delegates to anima_cache.py) -----------------


# Namespace constants shared between backend and frontend
...
def _get_cached_image_session():
    ...
    if _cached_image_session is None or _cached_image_session.closed:
        ...
        _cached_image_session = aiohttp.ClientSession(
            connector=connector,
            headers={
...
async def clear_cache_api(request):
    ...
    if namespace:
        removed = get_cache(namespace).clear()
        return web.json_response({"success": True, "namespace": namespace, "removed": removed})
    else:
        all_namespaces = [
            CACHE_NAMESPACE_ARTIST,
            CACHE_NAMESPACE_CHARACTER,
            CACHE_NAMESPACE_CLOTHING,
            CACHE_NAMESPACE_BACKGROUND,
        ]
        total = 0
        for ns in all_namespaces:
            total += get_cache(ns).clear()
        return web.json_response({"success": True, "removed": total})


@PromptServer.instance.routes.get("/anima-tools/cache-stats")
async def cache_stats_api(request):
    """返回缓存统计信息。指定 ?namespace= 则只查该文件夹。"""
    namespace = request.query.get("namespace", "")
    if namespace:
        return web.json_response(get_cache(namespace).stats())

    # Aggregate across all known namespaces
    all_namespaces = [
        CACHE_NAMESPACE_ARTIST,
        CACHE_NAMESPACE_CHARACTER,
        CACHE_NAMESPACE_CLOTHING,
    ...
    for ns in all_namespaces:
        s = get_cache(ns).stats()
        namespaces.append(s)
        totals["file_count"] += s["file_count"]
        totals["total_size_bytes"] += s["total_size_bytes"]

    return web.json_response({
        "namespaces": namespaces,
        "file_count": totals["file_count"],
        "total_size_bytes": totals["total_size_bytes"],
        "total_size_mb": round(totals["total_size_bytes"] / (1024 * 1024), 2),
    })
